...
if page == "🏠 Dashboard Home":
    st.title("🧠 Alpha Mind")
    st.markdown("### AI-Powered Portfolio Intelligence Platform")
    st.divider()
    ...
    m4.metric("📊 Open Positions", summary["total_positions"])
    m5.metric("🎯 AI Predictions", len(predictions))
    ...
    if predictions:
        ...
        st.info("No predictions yet. Go to AI Predictions.")
...
# ══════════════════════════════════════════════════════
    ...
    st.markdown("*Real AI predictions tracked vs actual outcomes*")
    st.divider()
    ...
    with g2:
        portfolio = st.selectbox("Portfolio", ["SNIPER", "FORTRESS"])
    with g3:
        days = st.selectbox("Days Ahead", [7, 14, 21, 30], index=1)
    with g4:
        st.write("")
        st.write("")
        gen_btn = st.button(
    ...
    if "last_prediction" in st.session_state:
        ...
        if "error" in result:
            ...
            m2.metric("🎯 Target Price", f"${result['predicted_price']:.2f}", f"{change:+.1f}%")
            m3.metric("📈 Direction", f"{dir_icon} {direction}")
            m4.metric("🎲 Confidence", f"{conf:.0f}%")
            m5.metric("📅 Target Date", result.get("target_date", "N/A"))
    ...
    if not predictions:
        ...
        s2.metric("Active", sum(
            1 for p in predictions if p.get("status") == "ACTIVE"
        ))
        ...
        for pred in predictions:
            dir_icon = (
                "🟢 UP" if pred.get("predicted_direction") == "UP"
                else "🔴 DOWN"
            )
            port_icon = "⚡" if pred.get("portfolio") == "SNIPER" else "🏰"
            ...
            with st.container():
                ...
                with h1:
                    st.markdown(
                        ...
                        f"→ {dir_icon} → **${pred['predicted_price']:.2f}**"
                    )
                with h2:
                    st.caption(f"By: {pred.get('target_date')}")
                with h3:
                    if conf_pct >= 75:
                ...
                c1.metric("Entry", f"${pred['current_price_at_prediction']:.2f}")
                c2.metric(
                    "Now", f"${pred['current_price_now']:.2f}",
                    f"{pred['price_change_so_far_pct']:+.1f}%"
                )
                c3.metric("Target", f"${pred['predicted_price']:.2f}")
                c4.metric("Portfolio", pred.get("portfolio", "N/A"))
                ...
                with st.expander("🧠 Full AI Analysis"):
                    ...
                    with r1:
                        ...
                        st.write(pred.get("technical_summary", "N/A"))
                    with r2:
                        st.markdown("**🚀 Catalysts:**")
...
# ══════════════════════════════════════════════════════
    ...
    st.markdown("*Real-time data from Alpaca Paper Trading*")
    st.divider()
    ...
    m1, m2, m3, m4, m5 = st.columns(5)
    m1.metric("💰 Portfolio Value", f"${account.get('portfolio_value', 0):,.2f}")
    m2.metric("💵 Cash", f"${account.get('cash', 0):,.2f}")
    m3.metric("⚡ Buying Power", f"${account.get('buying_power', 0):,.2f}")
    daily_pl = account.get("daily_pl", 0)
    daily_pl_pct = account.get("daily_pl_pct", 0)
    m4.metric("📈 Daily P&L", f"${daily_pl:,.2f}", f"{daily_pl_pct:+.2f}%")
    m5.metric("📋 Open Positions", summary["total_positions"])
    ...
    if positions:
        for pos in positions:
            ...
            p2.metric("Quantity", f"{pos['qty']:.0f} shares")
            p3.metric("Entry Price", f"${pos['avg_entry_price']:.2f}")
            p4.metric("Current", f"${pos['current_price']:.2f}")
            p5.metric("P&L", f"${pl:,.2f}", f"{plpc:+.1f}%")
            p6.metric("Market Value", f"${pos['market_value']:,.2f}")

            if st.button(f"❌ Close {pos['symbol']}", key=f"close_{pos['symbol']}"):
                result = services["broker"].close_position(pos["symbol"])
                if result["success"]:
                    st.success(f"✅ {pos['symbol']} closed!")
                    st.rerun()
                else:
                    st.error(f"❌ Failed: {result.get('error')}")
            st.divider()
    else:
        st.info("📭 No open positions yet. Use 'Place Paper Trade' to open your first trade.")

    st.subheader("📜 Recent Orders")
    if orders:
        for order in orders:
            side_icon = "🟢" if "buy" in str(order["side"]).lower() else "🔴"
            st.markdown(
...
# ══════════════════════════════════════════════════════
    ...
    st.warning("⚠️ Paper Trading Mode: No real money used.")
    st.divider()
    ...
    with col1:
        ...
        trade_side = st.selectbox("Order Side", ["BUY", "SELL"])
        trade_qty = st.number_input(
            "Quantity (shares)", min_value=1,
            max_value=1000, value=5, step=1
        )
        trade_portfolio = st.selectbox(
            "Assign to Portfolio", ["SNIPER", "FORTRESS"]
        )
        trade_reason = st.text_area(
            "Reason for Trade",
            placeholder="e.g. Strong momentum breakout...",
            height=100
        )
    ...
    with col2:
        ...
        if trade_ticker:
            ...
            if not df.empty:
                analysis = services["indicators"].get_full_analysis(
                    df, trade_ticker
                )
                if "error" not in analysis:
                    ...
                    st.metric("MACD", analysis["macd"]["signal"])
                    st.metric("Suggested Stop", f"${analysis['atr']['stop_loss']:.2f}")
                    st.metric("Suggested Target", f"${analysis['atr']['take_profit']:.2f}")
    ...
    if trade_ticker and trade_qty > 0:
        try:
            current_p = services["market"].get_current_price(trade_ticker)
            trade_value = current_p * trade_qty
            pct_of_portfolio = trade_value / portfolio_val * 100

            st.markdown(
                f"**Order Summary:** {trade_side} **{trade_qty}** "
                f"shares of **{trade_ticker}** @ ~${current_p:.2f}"
            )
            st.markdown(
            ...
            if execute_btn:
                with st.spinner(f"Placing {trade_side} for {trade_ticker}..."):
                    result = services["broker"].place_market_order(
                        symbol=trade_ticker,
                        qty=int(trade_qty),
...
# ══════════════════════════════════════════════════════
# PAGE 4: PENDING APPROVALS (FIXED)
# ══════════════════════════════════════════════════════
    ...
    trades = [
        {
            ...
            "reasoning": (
                "SPY showing strong momentum above VWAP. "
        ...
        },
        {
            ...
            "confidence": 0.79,
            "qty": 2,
            "agents": ["Fundamental ✅", "Macro ✅", "Technical ✅"],
            "reasoning": (
                "AAPL at key support level. "
                "Strong earnings growth trajectory. "
                "AI services revenue accelerating. "
                "Long term fortress position."
            )
        }
    ]

    st.warning(f"⏳ {len(trades)} trades waiting for your approval")
    ...
    for i, t in enumerate(trades):
        with st.container():
            ...
            c2.metric("Stop Loss", f"${t['stop']:.2f}", f"-${risk:.2f}")
            c3.metric("Target", f"${t['target']:.2f}", f"+${reward:.2f}")
            c4.metric("R:R", f"{rr}:1")
            c5.metric("Confidence", f"{t['confidence']*100:.0f}%")

            st.markdown(f"**Agents:** {' | '.join(t['agents'])}")
            ...
            if b1.button(
                ...
                with st.spinner(f"Executing {t['ticker']} trade..."):
                    result = services["broker"].place_market_order(
                        symbol=t["ticker"],
                        qty=t["qty"],
                        side=t["signal"].lower(),
            ...
            if b2.button("❌ REJECT", key=f"rej_{i}", use_container_width=True):
                st.error(f"❌ {t['ticker']} rejected.")

            if b3.button("⏰ +15 mins", key=f"wait_{i}", use_container_width=True):
                st.warning("⏰ Reminder set for 15 minutes.")

            if b4.button("🔍 Details", key=f"det_{i}", use_container_width=True):
                st.json(t)
...
# ══════════════════════════════════════════════════════
    ...
    if run_btn and aticker:
        with st.spinner(f"Analyzing {aticker}..."):
            df = services["market"].get_historical_bars(aticker, "6mo")

        if df.empty:
            st.error(f"No data for {aticker}")
        else:
            r = services["indicators"].get_full_analysis(df, aticker)
            if "error" in r:
                st.error(r["error"])
            else:
                st.subheader(f"📊 {aticker} Analysis")
                t1, t2, t3, t4 = st.columns(4)
                t1.metric("Price", f"${r['current_price']:.2f}")
                t2.metric("Trend", r["trend"])
                t3.metric("RSI", f"{r['rsi']['value']:.1f}", r["rsi"]["signal"])
                ...
                with d1:
                    ...
                    st.write(f"EMA 20: ${r['moving_averages']['ema_20']:.2f}")
                    st.write(f"EMA 50: ${r['moving_averages']['ema_50']:.2f}")
                    st.write(f"EMA 200: ${r['moving_averages']['ema_200']:.2f}")
                    gc = r["moving_averages"]["golden_cross"]
                    st.write(f"Golden Cross: {'✅' if gc else '❌'}")

                with d2:
                    st.markdown("**📊 Bollinger Bands**")
                    st.write(f"Upper: ${r['bollinger_bands']['upper']:.2f}")
                    st.write(f"Middle: ${r['bollinger_bands']['middle']:.2f}")
                    st.write(f"Lower: ${r['bollinger_bands']['lower']:.2f}")
                    st.write(f"Position: {r['bollinger_bands']['position']}")

                with d3:
                    st.markdown("**⚠️ Trade Levels**")
                    st.metric("ATR", f"${r['atr']['value']:.2f}")
                    st.error(f"🛑 Stop: ${r['atr']['stop_loss']:.2f}")
                    st.success(f"🎯 Target: ${r['atr']['take_profit']:.2f}")
                    st.info(f"⚖️ R:R: {r['atr']['risk_reward_ratio']}:1")
                ...
                fig.add_trace(go.Scatter(
                    x=df.index, y=e20, name="EMA 20",
                    line=dict(color="#4fc3f7", width=1.5)
                ))
                fig.add_trace(go.Scatter(
                    x=df.index, y=e50, name="EMA 50",
...
# ══════════════════════════════════════════════════════
    ...
    port_filter = st.selectbox(
        "Filter by Portfolio", ["ALL", "SNIPER", "FORTRESS"]
    )

    activities = [
        {
            "time": "09:47", "agent": "🐋 Whale Hunter",
            "action": "Detected $2.1M dark pool SPY @ current price",
            "level": "HIGH", "portfolio": "SNIPER"
        },
        {
            "time": "09:52", "agent": "📊 Technical",
            "action": "SPY RSI bullish, MACD crossover confirmed",
            "level": "HIGH", "portfolio": "SNIPER"
        },
        {
            "time": "09:58", "agent": "📰 Sentiment",
            "action": "Positive market sentiment. Score: 0.82",
            "level": "MEDIUM", "portfolio": "SNIPER"
        },
        {
            "time": "10:02", "agent": "🧠 Orchestrator",
            "action": "SPY package ready. 3/3 agree. 87%",
            "level": "HIGH", "portfolio": "SNIPER"
        },
        {
            "time": "10:02", "agent": "🛡️ Risk Warden",
            "action": "SPY passed all 7 checks. R:R=2:1",
            "level": "LOW", "portfolio": "SNIPER"
        },
        {
            "time": "10:03", "agent": "⏳ Human Gate",
            "action": "SPY BUY sent for your approval",
            "level": "HIGH", "portfolio": "SNIPER"
        },
        {
            "time": "10:15", "agent": "📊 Fundamental",
            "action": "AAPL earnings growth confirmed",
            "level": "MEDIUM", "portfolio": "FORTRESS"
        },
        {
            "time": "10:22", "agent": "🧠 Prediction",
            "action": "AAPL long term buy opportunity identified",
            "level": "HIGH", "portfolio": "FORTRESS"
        },
    ]
    ...
    filtered = [
        ...
        if port_filter == "ALL" or a["portfolio"] == port_filter
    ]
    ...
    for act in filtered:
        ...
        with c3:
            st.markdown(f"{icons.get(act['level'], '⚪')} {act['action']}")
        with c4:
            st.caption("⚡ S" if act["portfolio"] == "SNIPER" else "🏰 F")
        st.divider()

st.markdown("---")
st.caption(
    f"🧠 Alpha Mind | Paper Trading Mode | "
    f"Alpaca Connected ✅ | © {datetime.now().year}"
)